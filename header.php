...
<?php
    ...
    if(empty($themeNavbar)) { $themeNavbar ="navbar-dark bg-dark"; }
?>
					...
					<?php if ( get_option( 'users_can_register' ) ) : ?>
						<?php if ( is_user_logged_in() ) : ?>
							<li class="account-login">
                                    <span>
                                        <a href="<?php echo esc_url( get_permalink( get_option('woocommerce_myaccount_page_id') ) ); ?>"><?php esc_html_e( 'My Account', 'anero' ) ?></a>
                                        <small>/</small>
                                        <a href="<?php echo esc_url( wp_logout_url( get_permalink( get_option('woocommerce_myaccount_page_id') ) ) );?>"><?php esc_html_e( 'Logout', 'anero'); ?></a>
                                    </span>
							</li>
						<?php else: ?>
							<li class="account-login">
                                <span>
                                    <a class="account-link" href="<?php echo esc_url( get_permalink( get_option('woocommerce_myaccount_page_id') ) ); ?>"><?php esc_html_e( 'Login / Register', 'anero' ) ?></a>
                                </span>
							</li>
						<?php endif; ?>
					<?php endif; ?>
...
<?php
	if(get_theme_mod( 'set_theme_header') == 1) {
		require_once(get_template_directory().'/inc/header-1.php');
	}
	
	if(get_theme_mod( 'set_theme_header', 2) == 2) {
		require_once(get_template_directory().'/inc/header-2.php');
	}
?>